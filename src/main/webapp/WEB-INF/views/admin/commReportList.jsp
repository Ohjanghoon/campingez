	...
	<jsp:param name="title" value="캠핑이지" />
				...
				<div class="content-wrap">
					...
					<table class="table text-center">
						...
						<tbody>
							<c:if test="${not empty commReportList}">
								<c:forEach items="${commReportList}" var="comm">
									<tr>
										<td scope="row" rowspan="${fn:length(comm.reportList)}">1</td>
										<td scope="row" rowspan="${fn:length(comm.reportList)}">
											<a href="${pageContext.request.contextPath}/community/communityView.do?no=${comm.commNo}">
												${comm.commNo}
											</a> 
										</td>
										<td scope="row" rowspan="${fn:length(comm.reportList)}">${comm.userId}</td>
									<c:forEach items="${comm.reportList}" var="report" varStatus="vs">
										<c:if test="${vs.first}">
...
const deleteComm = (e) => {	
	...
	if(confirm(`[\${commNo}] 게시글에 대해 '삭제' 조치를 취하시겠습니까?`)) {
		document.updateFrm.commNo.value = commNo;
		document.updateFrm.type.value = "C";
